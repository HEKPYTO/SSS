def test_ssfs_wrapper_small():
    ...
    X, y = sklearn.datasets.make_classification(n_samples=200, n_features=20, n_informative=5, random_state=0)
    sel = SFFS(y=5, rho_u=0.2, tau=1.0, warmup_probes=20, warmup_card=5, delta=5, seed=1).fit(X, y, d=5)
    assert len(sel) == 5
...
def test_sss_cli_reuters_smoke():
    ...
    cmd = [
        sys.executable, "-m", "src.sss.cli",
        "--data", "anc/data/reuters_apte.arff",
        "--rr-train", "50", "--rr-test", "40",
        "--scaler", "void",
...
def test_sss_filter_fit():
    ...
    sffs = SFFS(y=10, y_back=5, rho_u=0.2, tau=0.0, warmup_probes=5, warmup_card=5, delta=2, seed=1, scaler="void")
...
def test_sffs_search_emits_solution():
    ...
    from src.sss.criteria import CountingCriterion, WrapperKnn
    from src.sss.prng import ss_srand
    from src.sss.sampled_step import SampledStep
    from src.sss.split import Split, cv_folds, rr_split_class
    ...
    class_size = [20, 20]
    flat = np.zeros(40 * 10, dtype=np.float64)
